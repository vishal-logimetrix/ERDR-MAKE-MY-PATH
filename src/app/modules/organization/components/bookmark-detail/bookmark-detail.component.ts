...
export class BookmarkDetailComponent implements OnInit {
    ...
    // if (confirmation) {
      for (let i = 0; i < this.bookmarks.length; i++) {
        if (this.questionDetails['id'] == this.bookmarks[i]['question']['id']) {
          this.networkRequest.delete(`/api/deletementorbookmark/${this.bookmarks[i]['id']}/`)
          .subscribe(
            data => {
  ...
  getBookmarks() {
    ...
    this.networkRequest.getWithHeaders(`/api/getmentorbookmarks/?exam=${this.examId}&subject=${this.subjectId}&chapter=${this.chapterId}`)
      .subscribe(
        data => {
