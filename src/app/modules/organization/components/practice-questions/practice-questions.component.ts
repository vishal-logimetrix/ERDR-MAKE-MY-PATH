...
export class PracticeQuestionsComponent implements OnInit {
  ...
  makePaper() {
    ...
    this.networkRequest.putWithHeaders(formData, `/api/makementorpractice/`).subscribe(
      data => {
        console.log("Questions found ", data);
        this.misc.hideLoader();
        this.fetchedQuestions = data['questions'];
        var text = 'Your Practice is successfully shared to batch ' + this.batchDetails['name'] + ' !';
  ...
  findReplacementQuestion() {
    ...
    this.replaceSol = null;
    this.replacementQuestion = null;
    this.networkRequest.getWithHeaders(`/api/findpracticereplacementquestion/?question=${this.questionDetails['id']}&tempobj=${this.tmpobjId}`)
    .subscribe(
      data => {
        //@ts-ignore
        if (data.length > 0) {
  ...
  replaceQuestion() {
    ...
    this.networkRequest.putWithHeaders(formData, `/api/replacepracticequestion/`)
    .subscribe(
      data => {
  ...
  getTemporaryQuestionDetails() {
    this.currentQuestion = 1;
    this.questionIds = [];
    this.networkRequest.getWithHeaders(`/api/findmentortemppracticequestions/${this.tmpobjId}/`)
    .subscribe(
      data => {
