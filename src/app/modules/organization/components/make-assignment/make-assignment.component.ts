...
@Component({
  selector: 'app-make-assignment',
  templateUrl: './make-assignment.component.html',
  styleUrls: ['./make-assignment.component.scss']
})
export class MakeAssignmentComponent implements OnInit {
  ...
  fetchQuestions() {
    ...
    this.networkRequest.putWithHeaders(formData, `/api/findmentorpracticequestions/`).subscribe(
      data => {
        this.submitStatus = false;
        ...
        this.router.navigate([`/organization/practice-questions/${this.batchId}`],{
          queryParams: {
            tmpobj: data['id']
          }
        });
