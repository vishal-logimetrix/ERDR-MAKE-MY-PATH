...
@Component({
  ...
  styleUrls: ['./navbar-website.component.scss'],
...
export class NavbarWebsiteComponent implements OnInit {
    ...
    this.activeRoute.queryParams.subscribe(param => {
      if (param.pageSec) {
        ...
        if (param.pageSec == 'exams') {
          ...
          this.activeAssociation = false;
          this.activeFaq = false;
        }
        else if (param.pageSec == 'institute') {
          this.activeAssociation = true;
  ...
  loginUsingPassword() {
    ...
    if (username && password) {
      ...
      // console.log('user in stringiify object', JSON.stringify(user));
      this.auth.login(JSON.stringify(user), '/api/users/login/').subscribe(user => {
            console.log("user response form auth service", user);
            if (user['user']) {
              this.loaderActive = false;
  ...
  ngOnInit() {
    ...
    // Scroll to Feature view
    // this.scrollSubcription = this.misc.scrollToView.subscribe(viewName => {
    //   if (viewName === 'feature') {
    //     this.featureRef.nativeElement.scrollIntoView({ behavior: 'smooth', block: 'start' });
    //   }
    // });

    // Notify navbar to show feature menu
    this.misc.showMenuSubject.next('feature');
    this.startSlider();
  }
