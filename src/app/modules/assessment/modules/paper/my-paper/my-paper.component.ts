...
@Component({
  ...
  styleUrls: ['./my-paper.component.scss']
})
export class MyPaperComponent implements OnInit, OnDestroy {
...
closefullscreen(){
 ...
 this.isFullScreen = false;
  ...
  unloadHandler(event) {
    ...
    this.networkRequest.putWithHeaders(formData, `/api/resumepaper/`)
      .subscribe(
        data => {
          // console.log("time updatedaa", data);
          this.testObj['data']['assessmentpaperdetails']['pause_count'] = data['pause_count'];
        },
        error => {
    ...
    if (confirmation){
      ...
      .subscribe(
        data => {
          ...
          // this.getFAQs();
          this.networkRequest.getWithHeaders(`/api/assessmentpaper_questions/${this.paperId}/`).subscribe(
            data1 => {
              console.log("assessmentpaper_questions", data1, this.currentQuestion);
  ...
  startBlocking() {
    ...
    this.networkRequest.putWithHeaders(formData, `/api/pausepaper/`)
      .subscribe(
        data => {
          console.log("updated time ", data);
          var text = '';
          if (this.testObj['data']['assessmentpaperdetails']['pause_count'] == 0) {
            text = "You have paused the paper for the first time and are left with one more time."
          }
          else {
            text = "You have paused the paper a second time and it was the last time you could do so."
          }
  ...
  unmark() {
    for (let i = 0; i < this.bookmarks.length; i++) {
      if (this.questionDetails['id'] == this.bookmarks[i]['question']['id']) {
        this.networkRequest.delete(`/api/deletetempbookmark/${this.bookmarks[i]['id']}/`)
        .subscribe(
          data => {
  ...
  stopBlocking() {
    ...
    .subscribe(
      data => {
        ...
        //   // this.router.navigateByUrl(`/assessment/dashboard/report/${this.testObj['paper_type']}/${this.testObj['id']}/1`);
        // }
        if (data['submitted'] && (data['remaining_time'] <= 0 || !data['remaining_time'])  ) {
          this.alertService.hideAlert().subscribe();
          this.test.completeTest();
        }
  ...
  bookmark() {
    ...
    this.networkRequest.putWithHeaders(formData, `/api/savebookmark/`)
    .subscribe(
      data => {
...
/**
 ...
 getTest() {
  ...
  .subscribe(
    data => {
      console.log("paper detailsaa ", data);
      this.paperDetails = data;
      this.Noripple = true;
      this.ripple = false;
      
      this.testObj = data;
      console.log("paper testObj ", this.testObj);
      this.subjects = data['subjects'];
      // if (this.paperDetails['submitted'] && (this.paperDetails['remaining_time'] <= 0 || !this.paperDetails['remaining_time'])  ) {
      //   this.alertService.hideAlert().subscribe();
      //   this.test.completeTest();
      //   // this.router.navigateByUrl(`/assessment/dashboard/report/${this.testObj['paper_type']}/${this.testObj['id']}/1`);
      // }
...
fetchBookmarksandUpdate() {
    ...
    this.networkRequest.getWithHeaders(`/api/assessmentpaper_questions/${this.paperId}/`).subscribe(
      data => {
        ...
        if (data['exam_status']) {
          ...
          //   // this.test.testObj = data['assessmentpaperdetails'];
          //   this.alertService.hideAlert().subscribe();
          //   this.test.completeTest();
          // }, 100);
          return;
        ...
        this.examDetails = data['assessmentpaperdetails']['learner_exam']['exam'];
        this.testObj = data['assessmentpaperdetails'];
  ...
  timer(time: number) {
    ...
    if (!this.paused && (this.testObj['id'] == this.paperDetails['id']) && !(this.paperDetails['submitted'])) {
  ...
  showQuestion() {
    // Intialize MathJax
    // console.log("showques", this.bookmarks);
    this.Noripple = true;
    this.misc.initializeMathJax(this.mathjax.nativeElement).subscribe();
    this.currentQuesBookmarked = false;
  ...
  generateUserAnswer(question: object, event: any) {
    ...
    if (question['type_of_question'] === 'mcq' || question['type_of_question'] === 'mcc' || question['type_of_question'] === 'assertion') {
      if (this.mcqDetails.length > 1) {
       ...
       if (event.target.elements.option.checked) {
        selectedAnswers.push(event.target.elements.option.id);
       }
     }
