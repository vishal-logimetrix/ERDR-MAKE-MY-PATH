...
import { PreGoalService } from 'src/app/services/pre-goal.service';

@Component({
  selector: 'app-pre-goal-assessment',
  templateUrl: './pre-goal-assessment.component.html',
  styleUrls: ['./pre-goal-assessment.component.scss']
})
export class PreGoalAssessmentComponent implements OnInit {
...
closefullscreen(){
  ...
  unloadHandler(event) {
    ...
    this.networkRequest.putWithHeaders(formData, `/api/resumegoalpaper/`)
      .subscribe(
        data => {
    ...
    if (confirmation){
      ...
      .subscribe(
        data => {
          ...
          // this.getFAQs();
          this.networkRequest.getWithHeaders(`/api/goalassessmentpaper_questions/${this.paperId}/`).subscribe(
            data1 => {
              console.log("goalassessmentpaper_questions", data1, this.currentQuestion);
  ...
  startBlocking() {
    ...
    this.networkRequest.putWithHeaders(formData, `/api/pausegoalpaper/`)
      .subscribe(
        data => {
 ...
 getTest() {
  ...
  .subscribe(
    data => {
      ...
      this.goalId = this.paperDetails['goal']['id'];
      this.Noripple = true;
      this.ripple = false;
...
// }
  ...
  //  this.misc.showLoader();
    this.networkRequest.getWithHeaders(`/api/goalassessmentpaper_questions/${this.paperId}/`).subscribe(
      data => {
        console.log("goalassessmentpaper_questions", data);
        // Check If Assessment Exam is available
        this.Noripple = true;
        this.ripple = false;
