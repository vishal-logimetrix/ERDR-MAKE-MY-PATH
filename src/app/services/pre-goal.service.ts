...
export class PreGoalService {
  ...
  completeTest() {
    ...
    if (navigator.onLine) {
      ...
      submitTestApi = `/api/goalassessmentpaper_questions/${this.testObj['id']}/`;
      ...
      this.networkRequest.postWithHeader('', submitTestApi).subscribe(
        data => {
          ...
          if (this.testObj['paper_type'] == 'paper') {
            this.router.navigateByUrl(`/assessment/dashboard/goal-report/${this.testObj['paper_type']}/${this.testObj['id']}/1`);
          }
          else {
  ...
  postCompletionProcessing() {
    this.networkRequest.postWithHeader('', `/api/process_goalassessmentpaper_postsubmit/${this.testObj['id']}/`).subscribe(
      data => {
  ...
  submitAnswers(userResponse) {
    ...
    return new Observable(observer => {
      if (this.remainingSeconds > 0) {
        ...
        this.networkRequest.postFormData(userResponse, '/api/post_goal_paper_answer/').subscribe(
          data => {
            observer.next(data);
