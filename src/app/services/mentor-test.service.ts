...
export class MentorTestService {
  ...
  completeTest() {
    ...
    if (navigator.onLine) {
      ...
      submitTestApi = `/api/mentorassessmentpaper_questions/${this.testObj['id']}/`;
      ...
      this.networkRequest.postWithHeader('', submitTestApi).subscribe(
        data => {
          ...
          if (this.testObj['paper_type'] == 'paper') {
            ...
            this.router.navigateByUrl(`/assessment/dashboard/assignment-report/${this.testObj['id']}`);
          }
  ...
  postCompletionProcessing() {
    this.networkRequest.postWithHeader('', `/api/process_mentorpaper_postsubmit/${this.testObj['id']}/`).subscribe(
      data => {
  ...
  submitAnswers(userResponse) {
    ...
    return new Observable(observer => {
      if (this.remainingSeconds > 0) {
        ...
        this.networkRequest.postFormData(userResponse, '/api/post_answer_mentorpaper/').subscribe(
          data => {
            observer.next(data);
